...
RayTracingDriver::~RayTracingDriver() {
  ...
  freeWorld<<<1, 1>>>(dataList_, world_);
  checkCudaErrors(cudaDeviceSynchronize());
  checkCudaErrors(cudaFree(world_));
  checkCudaErrors(cudaFree(dataList_));
...
void RayTracingDriver::init(Texture2D &texture, int screenWidth,
  ...
  }
  {
    ...
    checkCudaErrors(
        cudaMalloc((void **)&dataList_, numHitables * sizeof(Hitable *)));
    checkCudaErrors(cudaMalloc((void **)&world_, sizeof(Hitable *)));
    createWorld<<<1, 1>>>(dataList_, world_, worldRandState_);
    checkCudaErrors(cudaGetLastError());
    checkCudaErrors(cudaDeviceSynchronize());
  }
}
void RayTracingDriver::render(Camera &camera) {
  ...
  dim3 blockDim(BLOCK_SIZE, BLOCK_SIZE, 1);
  {
    ...
    raytracingRendering<<<gridDim, blockDim>>>(
        ...
        cameraRT, world_, this->pixelRandState_, camera.dirty_,
        renderingAccTimes_);
    camera.dirty_ = false;
  }
  checkCudaErrors(cudaGetLastError());
