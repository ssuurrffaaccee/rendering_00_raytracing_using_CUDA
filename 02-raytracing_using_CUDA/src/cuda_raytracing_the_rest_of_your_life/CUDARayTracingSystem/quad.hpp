...
class Quad : public Hittable {
 public:
  ...
  __device__ float pdf_from(const Point3& origin,
                             const Vec3& sample_direction, curandState *local_rand_state) const override {
    HitRecord rec;
  ...
  __device__ Vec3 sample_from(const Vec3& origin, curandState *local_rand_state) const override {
    auto p = Q_ + (random_float(local_rand_state) * u_) + (random_float(local_rand_state) * v_);
    return p - origin;
  }
